...
#ifndef DRIVE_RECORDER_SER_APP_H
#define DRIVE_RECORDER_SER_APP_H
...
private:
  std::map<Address, std::pair<struct Location, Ptr<Socket> > > cliInfo;
  std::map<Address, struct ReportInfo> reportInfo;
  //std::map<Address, Ptr<Socket> > socketMap;

private:
  std::vector<Ptr<Socket> > GetNearbyCli (Address address, Location loc);
  void SendQueryToCli (std::vector<Ptr<Socket> > sockets, double time);
...
#endif /* DRIVE_RECORDER_SER_APP_H */
